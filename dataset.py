...
def min_max_scale(coords, min_val=None, max_val=None):
# 如果最小值和最大值没有提供，从数据中计算它们
...
class PoiDataset(Dataset):
    ...
    def __init__(self, users, times, time_slots, coords, locs, sequence_length, batch_size, split, usage, loc_count,
        ...
        self.loc_coords = {key: 0 for key in range(loc_count)}
        self.sequences_freq = []
        self.sequences_lbl_freq = []

        self.matrix = np.zeros((loc_count, loc_count))
        self.sequences_trans = []
        ...
        for i, loc in enumerate(locs):
            ...
            for j, location in enumerate(loc[:train_thr]):
                self.freq[location] += 1
                if j==0:
                    temp = location
                else:
                    self.matrix[temp, location] += 1
                    temp = location           
    ...
    def __getitem__(self, idx):
        ...
        return x, t, t_slot, s, y, y_t, y_t_slot, y_s, reset_h, torch.tensor(self.active_users), f, y_f
